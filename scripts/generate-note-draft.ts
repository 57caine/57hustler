...
【記事の要件】
文字数: 5000〜6500字（SEO最適化のため十分な文量を確保。各セクションを充実させること）
対象読者: 占いが好きな20〜50代の女性。九星気学や易経を知らない初心者。読んで「へ〜！」「面白い！」と感じてほしい。

【構成】
1. タイトル（読者が思わずクリックしたくなる、今週のテーマを凝縮した一言を添える形式）

2. 今週の流れ（400字以上）
   - 難しい言葉なしにわかりやすく解説
   - 自分の体験談・身近なたとえを使って親しみやすく
   - 「今週はこう動くといい」という具体的な行動ヒントを1つ入れる

3. 九星別 今週の運勢（全9星、各200字以上）
   - 「〇〇星（XX・XX年生まれの方）は〜ですね」形式で生年を必ず記載
   - 恋愛・仕事・金運の3軸＋今週の一言アドバイス
   - その星ならではの個性に触れた具体的な描写を入れる

4. 今週の易経豆知識コーナー（600字以上）
   - 第${hexagram.number}卦「${hexagram.name}」の解説
   - 易経が生まれた背景・歴史的エピソード（具体的な人物・時代を交えて）
   - 「実はあの有名人も易経を活用していた」「ニールス・ボーアが紋章に太極図を採用した理由」のような、読者が思わず人に話したくなるトリビアを必ず1〜2個入れる
   - 現代のビジネス・人間関係への具体的な活かし方

5. 九星気学 深掘り豆知識（500字以上）
   - 今週の年盤・月盤の星にまつわる歴史的・文化的なトリビア
   - 「なぜ9という数字なのか」「洛書（らくしょ）とは何か」「マジックスクエアと九星の関係」など、知的好奇心を刺激するエピソード
   - 世界の類似思想との比較（風水・数秘術・カバラなど）があれば触れる
   - 難しくなりすぎず、「へ〜！そうなんだ！」で終わる読み心地に

6. 今週の行動アドバイス3選（各アドバイスに200字以上の解説）
   - 「〜してみてください」という口調
   - なぜそれが効くのか、九星気学・易経の根拠を具体的に説明
   - 明日から実践できる具体的な行動まで落とし込む

7. Threads連携（自然な流れで）
   「毎日の運勢は @westin_lab（Threads）で毎朝7時に無料でお届けしています。ぜひフォローしてみてください🌟」

8. 締めのCTA
   「より深く知りたい方は、有料マガジン『westin_labの九星気学鑑定』へどうぞ。吉方位・月間カレンダーも毎週更新しています。月500円で読み放題です💫」

【文章の質に関するルール】
- 各セクションに必ず1つ以上「へ〜！」「知らなかった！」と言いたくなる豆知識・トリビアを入れること
- 歴史的人物・有名人・科学者のエピソードを使うときは名前・時代・背景を具体的に書く
- 「実は〜」「意外にも〜」「ここだけの話」など読者の好奇心を引く書き出しを積極的に使う
- 専門用語は必ずやさしく補足する（例：「洛書（ろくしょ）＝約4000年前に中国で発見されたとされる魔方陣」）
- 口調: 「〜ですね」「〜ですよ」「〜かもしれません」「〜してみてください」。丁寧だけど距離が近い温かさ
- 怪しい・神秘的すぎる表現は避ける。知的で信頼できる占い師の言葉を意識する
