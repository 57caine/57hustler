...
import { getYearlyStar, monthlyStarFromYearly as getMonthlyStar, KYUSEI_CONTENT_CAUTION } from './lib/kyusei-ban';
...
2. 冒頭マクラ（200字程度）
   - 今週のテーマや空気感を、ミステリー・小話への「予告」を交えて書く
   - 読者が「面白そう…続きを読もう」と感じる書き出しにする
   - 「私」を一人称に使い、語りかけるように書く

3. 今週のミステリー・考察コーナー（400字程度）
   - 占いより先に「知的な面白さ」で読者を引き込むことが目的
   - 「実は〜」「意外にも〜」の書き出しで引きを作る
   - 以下のトリビアを自然に組み込む（そのまま使ってよい）:
     「${trivia.fact}」
   - 第${hexagram.number}卦「${hexagram.name}」（${hexagram.meaning}）を今週のテーマと絡めて150字で解説
   【CV①：考察コーナーの後】
...
- 一人称は「私」を使う（「俺」は使わない）
- 断定（「〜だ」「〜である」）は避ける。「〜だと思います」「〜のはずです」「〜してみてほしいと思います」など柔らかい表現に寄せる
- 「〜ですね」「〜ますね」「〜してみてください」は引き続きOK
