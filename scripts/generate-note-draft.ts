...
async function generateArticle(): Promise<string> {
  ...
  const prompt = `あなたは九星気学と易経の専門家で、noteで占いマガジンを運営する人気占い師です。
...
文字数: 2500〜3500字
構成:
1. タイトル: 「【九星気学×易経】今週の運勢〜${weekRange}〜」形式で、読者が思わず開く一言を添える
2. 今週のエネルギー解説（年盤・月盤・易経の卦の組み合わせからこの週のテーマを解説 300字）
3. 九星別 今週の運勢（全9星、各150字程度。恋愛・仕事・金運の3軸で）
4. 今週の易経コーナー: 第${hexagram.number}卦「${hexagram.name}」の詳細解説と今週への応用（400字）
5. 今週の開運アクション（全星座共通・3項目）
6. Threads連携：「毎日の日盤運勢は @westin_lab（Threads）で毎朝7時に無料発信中」
7. 出口戦略CTA（以下の2つを自然に組み込む）:
   【有料マガジン誘導】「より詳しい鑑定・吉方位活用・月間カレンダーは有料マガジン『westin_labの九星気学鑑定』で毎週更新。月額500円で見放題」
   【個別相談誘導】「あなたの本命星・月命星から算出するパーソナル鑑定も受け付け中。生年月日を添えてnoteのコメントまたはDMへ。料金：¥5,000〜（60分）」

トーン: 専門的だが親しみやすい。読者の背中を押す言葉を多く使う。
...
記事本文のみ出力（前置き不要）。`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 5000,
    system: '九星気学・易経の専門家兼noteクリエイターです。読者の役に立ち、保存・シェアされやすい鑑定記事を書きます。Threads(@westin_lab)との連動を意識した一貫した世界観を維持します。',
