...
async function generateArticle(): Promise<string> {
  ...
  const prompt = `あなたは九星気学と易経が大好きな占い師で、noteで占いマガジンを書いています。

【今週の鑑定データ】
期間: ${weekRange}
年盤の中宮星: ${yearlyStar.name}（${yearlyStar.element}・テーマ：${yearlyStar.theme}）
月盤の中宮星: ${monthlyStar.name}（${monthlyStar.element}・テーマ：${monthlyStar.theme}）
...
構成:
1. タイトル: 「今週の運勢まとめ〜${weekRange}〜✨」形式で、読者が思わず開く一言を添える
2. 今週のエネルギー解説（今週はどんな週なのかを、難しい言葉なしに300字でざっくり解説）
3. 九星別 今週の運勢（全9星、各150字程度。恋愛・仕事・金運の3軸で。「〇〇星の人は〜」という話しかけるような書き方で）
4. 易経コーナー:「今週のお告げは…？」というノリで第${hexagram.number}卦「${hexagram.name}」を身近な言葉で解説（400字）
5. 今週みんなに共通のラッキーアクション（3項目、「〜してみてね！」という口調で）
6. Threads連携：「毎日の運勢は @westin_lab（Threads）で毎朝7時に無料投稿してるよ〜！ぜひフォローしてね🌟」
7. 締めのCTA（以下を自然に組み込む）:
   【有料マガジン誘導】「もっと詳しく知りたい人は有料マガジン『westin_labの九星気学鑑定』へ！吉方位・月間カレンダーも毎週更新してるよ。月500円で読み放題💫」

専門用語は使わない。「日盤」「月盤」「年盤」「五行」などは出てきたとしても必ずやさしい言葉で説明を添える。
「友達に話しかけるような口調」「共感・ほっこり・背中を押す言葉」を大切に。
...
記事本文のみ出力（前置き不要）。`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 5000,
    system: '占いが大好きな親しみやすい占い師です。20〜50代の占い初心者女性に向けて、専門用語を使わず「友達に話しかけるような口調」で運勢を伝えます。難しい言葉は使わず、共感・背中を押す・ほっこりする表現を大切にします。Threads(@westin_lab)との連動を意識した一貫した世界観を維持します。',
