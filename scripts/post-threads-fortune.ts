...
`;
  ...
  const prompt = `あなたは九星気学の専門家です。以下のデータに基づき、Threads投稿文を作成してください。

${context}

【厳守事項】
- 投稿全体を必ず450文字以内に収める（超えるとAPIエラー）
- 冒頭：🔯 九星気学 今日の運勢【${dateStr}】
- 日盤の星名を明記する
- 各運勢（全体・金・恋・仕事）を絵文字つきで1行ずつ
- 吉方位とラッキーカラーを1行で
- 末尾：詳細はnoteで🌟 #九星気学 #今日の運勢
- データに忠実に。創作・誇張は不要
...
本文のみ出力（前置き不要）。`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 600,
    system: '九星気学の専門家です。提供されたデータに基づいて正確な運勢を簡潔に伝えます。',
