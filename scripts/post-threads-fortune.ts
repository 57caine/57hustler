...
/** 日盤の中宮星 (2024-01-06=一白基準、逆行) */
function getDailyStar(): number {
  const ref = new Date('2024-01-06T00:00:00Z');
  const now = new Date();
...
async function generateFortuneText(): Promise<string> {
  ...
  const context = `
【今日の日付】${dateStr}
【年盤の中宮星】${yearlyStar.name}（${yearlyStar.element}・${yearlyStar.direction}）
【日盤の中宮星】${dailyStar.name}（${dailyStar.element}・${dailyStar.direction}）
【日の星のキーワード】${dailyStar.keywords.join('・')}
【日の各運勢傾向】
...
【絶対守ること】
- 全体450文字以内（超えると投稿できない）
- 冒頭：🔯 今日の運勢【${dateStr}】
- 「九星気学」「日盤」「五行」などの専門用語は使わない
- 「〜ですね」「〜ですよ」「〜してみてください」という丁寧で温かい口調
- 各運勢（全体・お金・恋愛・仕事）を絵文字つきで1行ずつ、短く
- 末尾：詳細はnoteで🌟 #今日の運勢 #占い
...
本文のみ出力（前置き不要）。`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 600,
    system: '30歳まで鳴かず飛ばず、九星気学の吉方位参拝で人生が逆転した経験を持つ、ちょっと不思議なおじさん占い師です。目に見えない力と宇宙の仕組みを探り続けています。20〜50代の占い初心者の方に向けて、専門用語は使わず「〜ですね」「〜ですよ」「〜してみてください」という丁寧で温かい口調で語りかけます。押しつけがましくなく、でも少しだけ深みのある言葉を大切にします。',
